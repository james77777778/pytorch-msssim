...
def get_argparser():
    ...
    parser.add_argument("--ckpt", default=None, type=str,
                        help="path to trained model. Leave it None if you "
                             "want to retrain your model")
    parser.add_argument("--gpu_id", type=str, default='0', help="GPU ID")

    parser.add_argument("--loss_type", type=str, default='ssim',
                        choices=['ssim', 'ms_ssim'])
...
def main():
    ...
    train_loader = data.DataLoader(
        ...
        batch_size=opts.batch_size, shuffle=True, num_workers=2,
        drop_last=True)
    ...
    print("Train set: %d, Val set: %d" % (len(train_loader.dataset),
...
def test(opts, model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for i, images in enumerate(val_loader):
            ...
            if i == 20:
                outputs = TF.to_pil_image(outputs.squeeze(0).detach().cpu())
                outputs.save('recons_%s.png' % (opts.loss_type))
