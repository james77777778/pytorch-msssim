...
class GDN(nn.Module):
    def __init__(self,
        ...
        beta_init = torch.sqrt(
            torch.ones(num_features, dtype=torch.float) + self.pedestal)
        gama_init = torch.sqrt(
            torch.full((num_features, num_features),
                       fill_value=gamma_init,
                       dtype=torch.float)
    ...
    def forward(self, x):
        gamma = self._reparam(self.gamma, self.gamma_bound)
        # expand to (C, C, 1, 1)
        gamma = gamma.view(self.num_features, self.num_features, 1, 1)
